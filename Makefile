...
ifeq ($(SIM_SERVER),)
	...
	rsync -avz --delete --exclude .git $(NESCTRL_DIR) $(SIM_USER)@$(SIM_SERVER):$(USER)/$(REMOTE_ROOT_DIR)
...
ifeq ($(FPGA_SERVER),)
	...
	rsync -avz --delete --exclude .git $(NESCTRL_DIR) $(FPGA_USER)@$(FPGA_SERVER):$(REMOTE_ROOT_DIR)
